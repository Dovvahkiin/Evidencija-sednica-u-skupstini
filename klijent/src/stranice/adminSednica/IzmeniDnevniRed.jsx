...
import { ValidacijaDnevnogReda } from "../../skripte/validacije/Validacije";

function IzmeniDnevniRed() {
  const { id } = useParams();
  const [TekstTacke, PostaviDnevniRed] = useState("");
  const { IzmenaDnevnogReda } = DnevniRedHook();
  ...
  const PrihvatanjeForme = async (e) => {
    ...
    try {
      await IzmenaDnevnogReda(TekstTacke, id);
    } catch (greska) {
      return greska;
    } finally {
  ...
  useEffect(() => {
    if (postojecaSednica && postojecaSednica.ID !== undefined) {
      PostaviDnevniRed(String(postojecaSednica.DnevniRed));
    }
  }, [PostaviDnevniRed, postojecaSednica]);
    ...
    <main>
      ...
      {postojecaSednica ? (
        <form className="okvirSajta" onSubmit={PrihvatanjeForme}>
          <h1>Izmeni dnevni red za sednicu pod brojem: {id}</h1>
...
export default IzmeniDnevniRed;
