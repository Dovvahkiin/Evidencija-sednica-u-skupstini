...
} = require("../models/SednicaModel.js");

const { PoziviValidacija } = require("../validacije/validacijaSednice.js");
const { DetaljiPodatakaClanova } = require("../BussinessLogic/kvorum.js");
...
class SednicaKontroler {
  ...
  async KreirajSednicu(req, res) {
    ...
    const rezultatValidacije =
      await instancaValidacijaSednice.ValidacijaUnosa(podaciKreiranja);
    ...
    try {
      ...
      const proveraPrisutnih =
        await instancaDetalja.DaLiImaDovoljnoPrisutnih(BrojPrisutnih);
      if (!proveraPrisutnih) {
        noviStatus = 3;
      }
  ...
  async BrisanjeSednice(req, res) {
    try {
      ...
      const rezultatBrisanja =
        await instancaAkcijeSednice.ObrisiSednicu(IDSednice);
  ...
  async IzmenaSednice(req, res) {
    ...
    const rezultatValidacije =
      await instancaValidacijaSednice.ValidacijaIzmene(podaciIzmene);
    if (!rezultatValidacije.validacija) {
      return res
        .status(400)
        .json({ GreskaValidacije: true, greske: rezultatValidacije.greske });
